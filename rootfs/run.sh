...
[ "${WEBDAV_SYMLINK_ENABLE}" = "yes" ] && echo 'disable_symlinks off;' >> /etc/nginx/conf.d/http_parameters.conf

WEBDAV_PORT=${WEBDAV_PORT:-8080}

sed -i -r "s/listen .*;/listen ${WEBDAV_PORT};/" /etc/nginx/conf.d/default.conf
